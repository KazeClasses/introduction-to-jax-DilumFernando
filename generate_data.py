...
class PendulumSimulation(eqx.Module):
    ...
    def render_pendulum(
        ...
        plt.imshow(image.reshape(self.image_size, self.image_size), cmap='gray', aspect='auto')
        plt.colorbar() 
        plt.title("")
        plt.show()
        return image.reshape(self.image_size, self.image_size)

    def plot_pendulum_frame(self, frame, title=""):
        plt.imshow(frame, cmap='gray', extent=(-1, 1, -1, 1))
        plt.colorbar()
        plt.title(title)
        plt.show()
    ...
    def generate_dataset(
        ...
        for i in range(n_sims):
            ...
            for t in range(0, len(frames), max(1, len(frames)//5)):  # Plot a few frames
                self.plot_pendulum_frame(frames[t], title=f"Frame {t}, Angle: {solution.ys[0][t]:.2f}")
